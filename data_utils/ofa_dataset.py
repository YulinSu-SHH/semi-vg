...
def collate_fn(samples, pad_idx, eos_idx):
    ...
    if samples[0].get("img_id", None) is not None:
        batch["img_id"] = [s["img_id"] for s in samples]
    if samples[0].get("ref", None) is not None:
        batch["refs"] = [s["ref"] for s in samples]
    ...
    if samples[0].get("w_resize_ratio", None) is not None:
        batch["w_resize_ratios"] = [s["w_resize_ratio"] for s in samples]
    if samples[0].get("h_resize_ratio", None) is not None:
        batch["h_resize_ratios"] = [s["h_resize_ratio"] for s in samples]
    # image generation
