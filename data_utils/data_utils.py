...
def read_text_general(file_path):
    if "s3://" in file_path:
        return read_text_ceph(file_path)
    else:
        with open(file_path, "r") as f_:
            text = f_.read()
        return text

def read_text_ceph(file_path):
    file_bytes = client.get(file_path)
    assert file_bytes is not None, f"Please check file at {file_path}"
