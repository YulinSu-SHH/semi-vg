...
class MyConcatDataset(torch.utils.data.ConcatDataset):
    ...
    def collate(self, samples):
        samples_v1 = []   # containing visual grounding data
        samples_v2 = []   #
        if isinstance(samples[0], (list, tuple)) and len(samples[0])==2:
        ...
        elif isinstance(samples[0], (list, tuple)) and len(samples[0])==1:
            for sample_tuple in samples:
                samples_v1.append(sample_tuple[0])
            res_v1 = collate_fn(samples_v1, pad_idx=self.tokenizer.pad_token_id, eos_idx=self.tokenizer.eos_token_id)
            return res_v1
            ...
            res_v1 = collate_fn(samples, pad_idx=self.tokenizer.pad_token_id, eos_idx=self.tokenizer.eos_token_id)
            return res_v1
