...
class OFAEncoder(OFAPreTrainedModel):
    ...
    def __init__(self, config: OFAConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        if config.resnet_model_path:
            ...
            model_dict=self.embed_images.state_dict()
            pretrained_dict = {k: v for k, v in resnet_state_dict.items() if k in model_dict}
            model_dict.update(pretrained_dict)
            self.embed_images.load_state_dict(model_dict)
