...
class AdjustLabelSmoothedCrossEntropyCriterion(_Loss):
    ...
    def forward(self, output, sample, update_num=0, reduce=True):
        ...
        if isinstance(sample, list):
            ...
            loss_v1,batch_loss_v1, sample_size_v1, batch_size_v1,logging_output_v1,batch_loss1_v2 = self.forward(output[0], sample[0], update_num, reduce)
            loss_v2, batch_loss_v2,sample_size_v2, batch_size_v2,logging_output_v2,batch_loss2_v2 = self.forward(output[1], sample[1], update_num, reduce)
            assert batch_size_v1==batch_size_v2
            batch_loss = torch.tensor(batch_loss_v1) + torch.tensor(batch_loss_v2)
            batch_loss.requires_grad_(True)
            batch_size=batch_size_v1
            loss = loss_v1 / sample_size_v1 + loss_v2 / sample_size_v2
            sample_size = 1
            batch_loss2 = torch.tensor(batch_loss1_v2)/sample_size_v1.cpu() + torch.tensor(batch_loss2_v2)/sample_size_v2.cpu()
            batch_loss2.requires_grad_(True)
            logging_output = {
                "loss": loss,
                "loss_v1": loss_v1,
                "loss_v2": loss_v2,
            ...
            return loss,batch_loss, sample_size,batch_size, logging_output,batch_loss2
        ...
        i_start=0
        i_end=sample_size[0]
        batch_loss=[torch.mean(loss[i_start:i_end])]
        batch_loss_v2=[torch.sum(loss[i_start:i_end])]
        for idx in range(1,len(sample_size)):
            i_start+=sample_size[idx-1]
            i_end+=sample_size[idx]
            single_loss=loss[i_start:i_end]
            batch_loss.append(torch.mean(single_loss))
            batch_loss_v2.append(torch.sum(single_loss))
        batch_loss = torch.tensor(batch_loss)
        batch_loss.requires_grad_(True)
        batch_size=len(sample_size)
        batch_loss_v2 = torch.tensor(batch_loss_v2)
        batch_loss_v2.requires_grad_(True)
        logging_output = {
            "loss": sum(loss),
        ...
        return sum(loss),batch_loss,sum(sample_size),batch_size, logging_output,batch_loss_v2
    ...
    def compute_loss(self, net_output, sample, update_num, reduce=True):
        ...
        int_mask = mask.long()
        batch_token = int_mask.sum(axis=1)
        ...
        return loss, nll_loss,batch_token
