...
def get_data_loader(args, logger):
    ...
    # Build the dataset.
    dataset_label = [dataset_class(args,
                            args.train_dataset[i],
                            is_label=True
                            ) for i in range(0,1)]
    dataset_unlabel = [dataset_class(args,
                            args.train_dataset[i],
                            is_label=False
                            ) for i in range(1,2)]
    
    logger.info(f"Dataset_label:{dataset_label},\tDataset_unlabel:{dataset_unlabel},")
    dataset_label = MyConcatDataset(args,dataset_label)
    dataset_unlabel = MyConcatDataset(args,dataset_unlabel)

    if args.num_epochs > 0:
        total_samples = len(dataset_label)+len(dataset_unlabel)
    ...
    sampler_label = DistributedSampler(dataset_label)
    sampler_unlabel = DistributedSampler(dataset_unlabel)
    ...
    # Torch data loader.
    train_data_loader_label = torch.utils.data.DataLoader(dataset_label,
                                                    sampler=sampler_label,
                                                    collate_fn=dataset_label.collate,
                                                    batch_size=args.batch_size,
                                                    drop_last=True,
                                                    num_workers=0,
                                                    pin_memory=True)
    train_data_loader_unlabel = torch.utils.data.DataLoader(dataset_unlabel,
                                                    sampler=sampler_unlabel,
                                                    collate_fn=dataset_unlabel.collate,
                                                    batch_size=args.batch_size,
                                                    drop_last=False,
                                                    num_workers=0,
                                                    pin_memory=True)
    ...
    eval_dataset_vg = RefcocoDataset(args,
                                 args.test_dataset,
                                 is_label=True,
                                 is_test=True)

    eval_sampler_vg = SequentialSampler(eval_dataset_vg)
    eval_dataset_gc = CaptionDataset(args,
                                 args.test_dataset,
                                 is_label=True,
                                 is_test=True)

    eval_sampler_gc = SequentialSampler(eval_dataset_gc)
    eval_data_loader_vg = torch.utils.data.DataLoader(
        eval_dataset_vg,
        sampler=eval_sampler_vg,
        collate_fn=eval_dataset_vg.collate,
        num_workers=0,
        pin_memory=True,
        batch_size=args.batch_size)
    eval_data_loader_gc = torch.utils.data.DataLoader(
        eval_dataset_gc,
        sampler=eval_sampler_gc,
        collate_fn=eval_dataset_gc.collate,
        num_workers=0,
        pin_memory=True,
        batch_size=args.batch_size)
    eval_data_loader=[eval_data_loader_vg,eval_data_loader_gc]


    return train_data_loader_label, train_data_loader_unlabel,eval_data_loader
